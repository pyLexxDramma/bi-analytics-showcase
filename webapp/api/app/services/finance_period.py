...
def build_finance_period_payload(
    *,
    tip_needle: str,
    ...
    """Aggregate 1C dannye plan/fact by period for ТипСтатьи / article markers."""
    view = view if view in {"monthly", "cumulative"} else "monthly"
    tip = tip_needle.casefold().replace("ё", "е")
    ...
    article = frame["СтатьяОборотов"].fillna("").astype(str)
    article_l = article.str.casefold().str.replace("ё", "е", regex=False)
    article_type = (
        frame["ТипСтатьи"].fillna("").astype(str).str.casefold().str.replace("ё", "е", regex=False)
    )

    if tip == "бдр":
        # Streamlit try_synthetic_bdr: marker «(БДР)» in article or type БДР;
        # if none present in dump — keep all rows (approx fallback).
        tip_mask = (
            article_l.str.contains(r"\(бдр\)|^бдр$|^бдр\s", regex=True)
            | (article_l.str.contains("бдр", regex=False) & ~article_l.str.contains("бддс", regex=False))
            | (article_type.str.contains("бдр", regex=False) & ~article_type.str.contains("бддс", regex=False))
        )
        if not bool(tip_mask.any()):
            tip_mask = pd.Series(True, index=frame.index)
    else:
        tip_mask = article_type.str.contains(tip, regex=False)
        # BDDS excludes BDR-marked articles
        tip_mask = tip_mask & ~(
            article_l.str.contains(r"\(бдр\)", regex=True)
            | article_l.str.strip().eq("бдр")
        )

    frame = frame[tip_mask].copy()
    before_lot = frame
    frame = frame[frame["СтатьяОборотов"].map(_turnover_article_has_lot_and_sublot).fillna(False)].copy()
    # BDR: if lot filter empties the frame, keep pre-lot rows (Streamlit behavior)
    if frame.empty and tip == "бдр" and not before_lot.empty:
        frame = before_lot.copy()
    if frame.empty:
        return _empty_payload(source=source, project=project, date_from=date_from, date_to=date_to, view=view)

    if "РасходДоход" in frame.columns:
    ...
    plan_mask = (
        scenario.str.contains("план", regex=False) & ~scenario.str.contains("факт", regex=False)
    ) | scenario.isin(plan_names)
    ...
    frame["period"] = (
        frame["period_date"].dt.month.map(lambda month: _MONTHS[month - 1])
        + " "
        + frame["period_date"].dt.year.astype(str)
    )
    ...
    period_rows_df = (
        grouped.groupby(["period_key", "period"], as_index=False)[["plan", "fact", "deviation"]]
        .sum()
        .sort_values("period_key")
    )
    if view == "cumulative":
        period_rows_df[["plan", "fact", "deviation"]] = period_rows_df[
            ["plan", "fact", "deviation"]
        ].cumsum()
    project_rows_df = (
        grouped.groupby("project", as_index=False)[["plan", "fact", "deviation"]]
        .sum()
    ...
    plan_sum = float(filtered.loc[filtered["kind"] == "plan", "amount"].sum())
    fact_sum = float(filtered.loc[filtered["kind"] == "fact", "amount"].sum())
    return {
