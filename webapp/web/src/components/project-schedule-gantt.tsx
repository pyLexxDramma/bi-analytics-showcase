...
export function ProjectScheduleGantt({
  ...
  const built = useMemo(() => {
    ...
    const planY = rows.map((_, i) => i + laneOffset("plan", hasFact && !labelPct));
    ...
    (Object.entries(buckets) as Array<[string, EdgeBucket]>).forEach(([key, bucket]) => {
      ...
      traces.push({
        ...
        hoverinfo: "skip",
        showlegend: false,
        // Даты только внутри области линий — не залезают в колонку имён.
        cliponaxis: true,
      });
    });
    ...
    // Запас слева под подписи дат начала внутри блока линий.
