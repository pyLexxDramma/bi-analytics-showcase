...
import { fetchBdds, type BddsPayload } from "@/lib/api";
...
const tableCell = "px-3 py-2 text-right tabular-nums text-tremor-content-strong dark:text-dark-tremor-content-strong";
...
export function BddsView() {
  ...
  const load = useCallback(async (nextFilters: Filters) => {
    ...
    setError(null);
    try {
      setData(await fetchBdds(nextFilters));
    } catch (cause) {
  ...
  return (
    <AppShell
      title="БДДС (расходы)"
      subtitle="План и факт расходов по оборотам 1С"
    >
      <Card className="mb-6 rounded-xl">
        <div className="grid gap-3 md:grid-cols-4">
          <label className="block text-sm">
            ...
            <select
              ...
              onChange={(event) => setFilters((state) => ({ ...state, project: event.target.value }))}
            >
          ...
          <label className="block text-sm">
            ...
            <select
              className="mt-1 w-full rounded-tremor-default border border-tremor-border bg-tremor-background px-3 py-2 text-tremor-default dark:border-dark-tremor-border dark:bg-dark-tremor-background"
              value={filters.view}
              onChange={(event) => setFilters((state) => ({ ...state, view: event.target.value as Filters["view"] }))}
            >
        ...
        <Text className="mt-3">
          Режим данных: <b>{data?.meta.data_mode ?? "…"}</b> · {loading ? "загрузка…" : `${data?.meta.rows ?? 0} строк`}
        </Text>
      </Card>
      ...
      <div className="space-y-6">
        <Grid numItemsSm={2} numItemsLg={4} className="gap-6">
          {kpiCards.map(([title, metric]) => (
            <Card key={title} className="rounded-xl">
              <Text>{title}</Text>
              <Metric className="mt-2 text-tremor-content-strong dark:text-dark-tremor-content-strong">{metric}</Metric>
            </Card>
          ))}
        </Grid>

        <Card className="rounded-xl">
          <Title className="!text-tremor-content-strong dark:!text-dark-tremor-content-strong">План и факт расходов</Title>
        ...
        <Grid numItemsLg={2} className="gap-6">
          <BddsTable title="По периодам" rows={data?.period_rows ?? []} label="period" />
          <BddsTable title="По проектам" rows={data?.project_rows ?? []} label="project" />
        </Grid>
      </div>
    </AppShell>
  );
...
function BddsTable({
  ...
  title: string;
  rows: Array<{ period?: string; project?: string; plan: number; fact: number; deviation: number }>;
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full text-left text-tremor-default">
          <thead className="bg-tremor-background-subtle text-tremor-label uppercase text-tremor-content dark:bg-dark-tremor-background-subtle dark:text-dark-tremor-content">
            <tr>
              <th className="px-3 py-2">{label === "period" ? "Период" : "Проект"}</th>
          ...
          <tbody className="bg-tremor-background dark:bg-dark-tremor-background">
            {rows.map((row, index) => (
              <tr key={`${row[label]}-${index}`} className="border-t border-tremor-border dark:border-dark-tremor-border">
                <td className="px-3 py-2 text-tremor-content-strong dark:text-dark-tremor-content-strong">{row[label]}</td>
